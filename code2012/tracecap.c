...
void save_registers(struct user_regs_struct *regs)
{
  ...
  regs->eip = *TEMU_cpu_eip; //程序计数器
  regs->xcs = TEMU_cpu_segs[R_CS].selector;
  regs->eflags = *TEMU_cpu_eflags; //标志寄存器
  regs->esp = TEMU_cpu_regs[R_ESP]; //栈顶寄存器
...
//开始跟踪***********************************************************************
//本函数是做好了trace的准备工作，并没有涉及如何撰写Trace文件。
int tracing_start(uint32_t pid, const char *filename) //（进程ID，文件名）
{
#ifdef INSN_INFO
  char infoname[128];
  if (infolog)
  ...
  strncpy(tracename, filename, 128); //filename->tracename
  ...
  setvbuf(tracelog, filebuf, _IOFBF, FILEBUFSIZE);//把tracelog与缓冲filebuf相关，按行
  if (0 == tracelog) {
    tracepid = 0;
    tracecr3 = 0;
    return -1;
  ...
  tracepid = pid; //进程id就是跟踪id
  tracecr3 = find_cr3(pid); //进程内核空间页表的地址
  if (0 == tracecr3) {
  ...
  /* Initialize hooks only for this process 单独为这个进程hook */
  temu_plugin->monitored_cr3 = tracecr3; //把该进程的cr3给monitor
  ...
  /* getrusage是linux的系统函数，意为get resourse usage
	RUSAGE_SELF=以当前进程的相关信息来填充rusage数据结构
	利用getrusage可以得到进程的相关资源信息。
	如：用户开销时间，系统开销时间，接收的信号量等等
  */
  return 0;
}
...
void tracing_stop()
{
  ...
  print_stats(); //在trace.c中实现，跟踪结束后输出跟踪的统计数据，4条
  ...
  if (getrusage(RUSAGE_SELF, &stopUsage) == 0) {
    ...
    double userProcessTime = (stopUT - startUT); //用户进程用时
    double systemProcessTime = (stopST - startST); //系统进程用时
    double processTime =  userProcessTime + systemProcessTime; //总的进程用时
